...
int currentFilter = 1; // defaults to lowpass, and lowpass SVFType is 1
...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size)
{
    ...
    if (hw.button1.RisingEdge())
    {
        ...
        tSVF_setFilterType(&myFilter, (SVFType)(currentFilter % 5)); // in this line, I'm "casting" the integer currentFilter, modulo 5 to get it between 0 and 4, to the type SVFtype so that the function will accept it. 
        // another example of casting would be truncating a float to an integer by writing something like (int)myFloat, if myFloat was already established as a float. The cast uses parenthesis around the new type and goes before the value you are casting.
    }
