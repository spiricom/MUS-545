...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size)
{
    ...
    for (size_t i = 0; i < size; i++)
    {
        ...
        tSimpleRetune_tuneVoice(&ps, 0, hw.knob1.Value() * 2.0f);
        mySample = tSimpleRetune_tick(&ps, mySample); //knob1 gives values from 0-1 and multiplying a signal by a value between 0 and 1 changes it's amplitude (volume) between totally off and fully on. 
...
int main(void)
{
    ...
    tSimpleRetune_init(&ps, 1, 80.0f, 700.0f, 1024, &leaf);
